...
var remote = (function(){
    ...
    function checkGitSource(id, source){
        if(source.url.match(remote.GITHUB_REGEX)){
            checkGitHubSource(id, source);
        }else{
            // TODO support Git sources other than github.com
            return handleRemoteVersionCheckError(id, "source.url '"+source.url+"' is not a valid github repo URL in the form 'https://github.com/username/reponame' or 'git://github.com/username/reponame.git'");
        }
    }

    function checkGitHubSource(id, source){
        function handleError(err){
            handleRemoteVersionCheckError(id, "Failed to read version from github repo", err);
        }

        try{
            var parts = source.url.match(remote.GITHUB_REGEX),
                protocol = parts[1],
                username = parts[2],
                password = parts[3],
                user = parts[4],
     ...
     * Public properties
     *********************/
    remote.GITHUB_REGEX = /^(https|git):\/\/(?:([^:@]*):?([^@:]*)@?)github\.com\/([^\/]+)\/([^\/.]+)(?:\.git)?$/;
